...
fn part_2 (input: &str) -> i64 {
    ...
    loop {
        ...
        (queue, finish) = find_options(&winds, max, &queue, goal);
        round += 1;
        if finish { // assumes first arrival on each is optimal
            println!("GOAL ##### {}", round);
            if iteration == 0 {
                goal = (0, -1);
                iteration += 1;
...
fn find_options(winds: &HashMap<(i64, i64), Vec::<char>>, max: (i64, i64), queue: &HashSet<(i64, i64)>, goal: (i64, i64)) -> (HashSet<(i64, i64)>, bool) {
    ...
    for o in queue {
        for d in directions {
            ...
            if x == 0 && y == -1 { //we can stay in start/end positions and wait for an opening
                options.insert((x,y));
            }
