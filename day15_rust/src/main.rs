...
fn main() {
    ...
    println!("Answer to Part 1 test: {}", part_1(&read_file("input copy.txt"), 10));
    println!("Answer to Part 1: {}", part_1(&read_file("input.txt"), 2_000_000));
    println!("Answer to Part 2 test: {}", part_2(&read_file("input copy.txt"), 20));
    println!("Answer to Part 2: {}", part_2(&read_file("input.txt"), 4_000_000));
}
...
fn count_empty_1(sensors: HashMap<(i32,i32), (i32, i32)>, row: i32) -> i32 {
    ...
    for (s, b) in sensors {
        ...
        if s.1 + manh >= row && s.1 - manh <= row { // ignore if no overlap
            if s.1 == row {// catch empty iterator
...
mod tests {
    ...
    #[test]
    fn test_p1_ex() {
        assert_eq!(part_1(&read_file("input copy.txt")), 26);
    }

    #[test]
    fn test_p1() {
        assert_eq!(part_1(&read_file("input.txt")), 4907780);
    }

    #[test]
    fn test_p2_ex() {
        assert_eq!(part_2(&read_file("input copy.txt")), 56000011);
    }

    #[test]
    fn test_p2() {
        assert_eq!(part_2(&read_file("input.txt")), 13639962836448);
    }
}